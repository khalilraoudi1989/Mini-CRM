...
{   
    ...
    public function destroy(Company $company)
    {
        ...
        if ($hasEmployees) {
            return redirect()->route('companies.index')->with('error', 'La société a des employés et ne peut pas être supprimée.');
        }

        // Supprimer la société seulement si elle n'a pas d'employés
        $company->delete();

        return redirect()->route('companies.index')->with('success', 'La société a été supprimée avec succès.');
    }
}
